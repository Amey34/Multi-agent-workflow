...
| `acf-contract-auditor-agent` | ACF schema/template contract validation | Catch and fix field/template mismatches before QA |
...
| `design-token-sync-agent` | Token normalization + sync | Keep styles aligned across Figma/React and ACF output |
| `debug-agent` | Root-cause debugging + minimal fixes | Fix broken rendering, logic, JS, or field mismatch issues |
| `figma-to-pen-agent` | Figma-to-Pencil bridge orchestrator | Convert Figma node/cache payload into editable `.pen` structure |
...
| `visual-regression-agent` | Screenshot parity validation | Detect and report visual drift by breakpoint |
...
| `acf-contracts` | Field JSON <-> template contract checks | Contract audit and schema-integrity workflows |
| `codex-executor` | Delegation workflow standard for Codex MCP tasks | `codex-*` command paths |
| `design-token-bridge` | Token translation framework from source design/code to theme variables | Token sync and style consistency workflows |
...
| `visual-parity` | Breakpoint-based parity and drift severity framework | Visual regression and design-faithfulness checks |
...
| `/figma-to-pen` | `<figma:<URL> \| cache:/abs/path.json> [pen:/abs/path/output.pen]` | Converts Figma source into `.pen` via figma + pencil MCP |
...
| `/post-pipeline-quality-gate` | `source:<...> scope:<block-slug|all> [site-url:...] [reference:...]` | Runs contract audit -> token sync -> visual regression (if URL present) |
| `/sync-design-tokens` | `<figma:URL|cache:/abs.json|react:/abs.tsx|react-dir:/abs/dir> [scope:block-slug|all]` | Syncs/normalizes tokens and reduces style drift |
| `/validate-acf-contracts` | `<block-slug|all>` | Audits and fixes ACF JSON/template contract mismatches |
| `/visual-regression-block` | `site-url:https://... scope:<block-slug|page|all> [reference:...]` | Runs breakpoint-level visual parity checks |

## Suggested Workflow

1. Build from design:
   - `/figma-block ...` for one block
   - `/figma-to-pen figma:...` to create/editable `.pen` baseline from Figma
...
5. Drift and contract control:
   - `/post-pipeline-quality-gate source:... scope:all [site-url:...]`
   - `/sync-design-tokens ...`
   - `/validate-acf-contracts all`
   - `/visual-regression-block site-url:... scope:...`
